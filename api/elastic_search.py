...
class FileIndex:
    ...
    def create_indice(self):
        ...
        self.es.indices.create(index=self.constants.CSEindex, ignore=400, body={"mappings" : {"properties" : {"text_content" : { "type" : "nested" }}}})
    ...
    def create_mapping(self):
        ...
        self.es.indices.put_mapping(
            ...
            doc_type= self.constants.CSEdoc_type,
            body={"mappings" : {"properties" : {"text_content" : { "type" : "nested" }}}}
        )

    def get_mapping(self):
        result = self.es.indices.get_mapping(index=self.constants.CSEindex, doc_type=self.constants.CSEdoc_type)
        pprint(result)
    ...
    def context_search(self, _query):
        res = self.es.search(index=self.constants.CSEindex, doc_type=self.constants.CSEdoc_type, body={ "query": { "nested": { "path": "text_content", "query": { "bool": { "must": [{ "match": { "text_content.content":  _query}}]}},"inner_hits":{}}}})
        return res
